...
column_names = ["CHROM","POS","REF","ALT","raw p-values (Chi2 contingency test)"]
...
def process(line):
    ...
    try:
        raw_pval = chi2_contingency([[i[0] for i in (vert, rouge, jaune)],
                                     [i[1] for i in (vert, rouge, jaune)]])[1]
        ...
        raw_pval = np.nan 
    newline = split_char.join([newline, str(raw_pval)])
